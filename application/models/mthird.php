...
class mthird extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->load->library( 'Plsql' );
        $sConfigFile = PACKAGE_CONFIG_PATH . 'mpokus' . EXT;
        $this->plsql->initialize( $sConfigFile );
    }
    ...
    public function getAllPokus( $iNum )
    {
        ...
        if( false === $this->plsql->insert( $aIn ) )
        {
            echo $this->plsql->getErrorMessage();
        }
        else
        {
            if( $this->plsql->PCUR_out->isEmpty() )
            {
                echo 'No data';
            }
            else
            {
                ...
                foreach ($this->plsql->PCUR_out->result() as $row)
                {
        ...
        $this->plsql->PCUR_out->free_result(); 
